...
import { AlertCircle, CheckCircle, Mail, User, Lock, Loader, Save, RefreshCw, Calendar } from 'lucide-react';
...
export function AccountSettingsAr({ accessToken, onLogout }: AccountSettingsArProps) {
  ...
  const handleUpdateUsername = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!/^[a-zA-Z0-9_-]+$/.test(newUsername)) {
        throw new Error('يمكن أن يحتوي اسم المستخدم على أحرف وأرقام وشرطات سفلية وواصلات فقط');
      }
  ...
  return (
    <div className="w-full space-y-6" dir="rtl">
      ...
      <Card className="p-6 bg-gradient-to-br from-white to-stone-50 shadow-lg border border-stone-200">
        <div className="flex items-center justify-between gap-4 mb-8 pb-6 border-b border-stone-200">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-gradient-to-br from-stone-300 to-stone-400 rounded-full flex items-center justify-center shadow-md">
              <User className="w-8 h-8 text-white" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-stone-900">معلومات الحساب</h2>
              <p className="text-sm text-stone-600 mt-1">إدارة بيانات حسابك الشخصية</p>
            </div>
          </div>
          <Button
            onClick={loadProfile}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-4 rounded-lg border border-stone-200 hover:border-stone-300 transition-colors">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide">البريد الإلكتروني الأخير</p>
            </div>
            <p className="text-lg font-semibold text-stone-900 break-all">{profile?.email || 'N/A'}</p>
            <p className="text-xs text-stone-500 mt-2">البريد المستخدم لتسجيل الدخول</p>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border border-stone-200 hover:border-stone-300 transition-colors">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide">اسم المستخدم الأخير</p>
            </div>
            <p className="text-lg font-semibold text-stone-900">{profile?.username || 'لم يتم التعيين'}</p>
            <p className="text-xs text-stone-500 mt-2">معرّف فريد لحسابك</p>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border border-stone-200 hover:border-stone-300 transition-colors">
            <div className="flex items-center gap-2 mb-2">
              <User className="w-4 h-4 text-stone-600" />
              <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide">الاسم</p>
            </div>
            <p className="text-lg font-semibold text-stone-900">{profile?.name || 'لم يتم التعيين'}</p>
            <p className="text-xs text-stone-500 mt-2">اسمك الكامل</p>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border border-stone-200 hover:border-stone-300 transition-colors">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide">عضو منذ</p>
            </div>
            <p className="text-lg font-semibold text-stone-900">
              {profile?.created_at ? new Date(profile.created_at).toLocaleDateString('ar-SA') : 'N/A'}
            </p>
            <p className="text-xs text-stone-500 mt-2">تاريخ إنشاء الحساب</p>
          </div>
        </div>

        {/* Latest Password Update Notice */}
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg flex items-start gap-3">
          <Lock className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
          <p className="text-sm text-blue-900">
            <strong>كلمة المرور الأخيرة:</strong> تم تحديثها بنجاح. يمكنك تغييرها في قسم "كلمة المرور" أدناه.
          </p>
        </div>
      </Card>
            ...
            <form onSubmit={handleUpdateUsername} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-stone-500 mt-1">
                  3 أحرف على الأقل، أحرف وأرقام وشرطات سفلية وواصلات فقط
                </p>
              </div>
